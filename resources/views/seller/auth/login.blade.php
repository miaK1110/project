...
@section('content')
    <div class="c-form">
        <form class="c-form__item-wrapper" method="POST" action="{{ route('seller.login') }}">
            <h2 class="c-form__title">店舗ログイン</h2>
            @csrf
            {{-- email form --}}
            ...
            <button type="submit" class="c-btn--primary--higher u-mb__s">
                {{ __('ログイン') }}
            </button>

            <a class="c-link__underline u-align__center" href="{{ route('seller.password.request') }}">
                {{ __('パスワードをお忘れですか？') }}
            </a>
        </form>
    </div>
